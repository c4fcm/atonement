...
Modified by Edward L. Platt <ed@elplatt.com> for the Atonement Project.
*/
...
if ( strtolower($rd_loop_method) != 'full' && strtolower($rd_loop_method) != 'excerpt' && strtolower($rd_loop_method) != "title" && strtolower($rd_loop_method != "atonement")) {
	$rd_loop_method = 'full';
}
...
///// -------------------------------------------------------------------------------------------///
	...
	switch ($rd_loop_method) {
		...
		case 'atonement':
			$x = 0;
			$y = 0;
			$rdscf_output .= '<div id="load_posts_container">';
			foreach($rdcsc_posts as $post) :
				setup_postdata($post);				
				$rdcsc_content = get_the_content();
				/// strip tag filters added for 1.2 release
				...
				$rdcsc_comments = get_comments();
				if($x == 2) {
					$rdscf_output.='<div class="home_post_box home_post_box_last" onmouseover="show_post_desc(' . $y . ')" onmouseout="hide_post_desc(' . $y . ')">';
				} else {
					$rdscf_output.='<div class="home_post_box" onmouseover="show_post_desc(' . $y . ')" onmouseout="hide_post_desc(' . $y . ')">';
				}
				$thumbnail = get_the_post_thumbnail($rdcsc_posts->ID, 'home-post',array('alt' => 'post image', 'class' => 'rounded'));
				if (empty($thumbnail)) {
					$thumbnail = '<img src="' . get_bloginfo('stylesheet_directory') . '/images/blog-image.jpg"/>';
				}
				$rdscf_output .= '<a href="' . get_permalink($rdcsc_posts ->ID) . '">' . $thumbnail . '</a>';
				$rdscf_output .= '<div class="home_post_desc" id="home_post_desc' . $y . '">';
				$temp_arr_content = explode(" ",substr(strip_tags(strip_shortcodes($rdcsc_content)),0,225));
				$temp_arr_content[count($temp_arr_content)-1] = "";
				$display_arr_content = implode(" ",$temp_arr_content);
				$rdscf_output .= $display_arr_content . '...';
				$rdscf_output .= '</div><!--//home_post_desc-->';
				$rdscf_output .= '<div class="home_post_title_cont">';
				$rdscf_output .= '<h3>' . get_the_title() . '</h3>';
				$rdscf_output .= '<h4>' . get_the_category_list(', ') . '</h4>';
				$rdscf_output .= '</div><!--//home_post_title_cont-->';
				$rdscf_output .= '</div><!--//home_post_box-->';
				...
				$y++;
			endforeach;
			$rdscf_output .= '</div><!--//load_posts_container-->';
			break;
	}
